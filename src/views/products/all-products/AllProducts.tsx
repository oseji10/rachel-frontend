...
const handleSubmit = async (event) => {
  ...
  try {
    ...
    if (response.status === 201) {
      const newProduct = await response.json(); // Get the newly created product from response
  ...
  <Box sx={modalStyle}>
    ...
    {selectedProduct && (
      <>
        ...
        <Typography variant="body1">
          <strong>Formulation:</strong> {selectedProduct?.formulation}
        </Typography>
        <Typography variant="body1">
          <strong>Manufacturer:</strong> {selectedProduct?.manufacturer?.manufacturerName}
        </Typography>
        <Typography variant="body1">
          <strong>Quantity:</strong> {selectedProduct?.quantity || 'N/A'}
        </Typography>
</>
    ...
    {selectedProduct && (
      <form
        onSubmit={async (e) => {
          ...
          try {
            ...
            fetchProducts(page); // Refresh data
          } catch (error) {
      ...
      >
        <TextField
          ...
          onChange={(e) => selectedProduct({ ...selectedProduct, productName: e.target.value })}
        />
        <TextField
          fullWidth
          margin="normal"
          label="Formulation"
          value={selectedProduct.formaulation}
          onChange={(e) => setSelectedPatient({ ...selectedProduct, formulation: e.target.value })}
        />
        <TextField
          fullWidth
          margin="normal"
          ...
          onChange={(e) => setSelectedProduct({ ...selectedProduct, manufacturerName: e.target.value })}
        />
